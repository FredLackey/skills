...
#
# Note: This is a system-wide setting, identical to the iTerm2 version.
# Running either script applies the change for all applications.
...
if [ "$NORMALIZED" = "0" ] || [ "$NORMALIZED" = "false" ] || [ "$NORMALIZED" = "no" ]; then
    ...
    echo "For Terminal.app: Cmd+Q then reopen."
fi
