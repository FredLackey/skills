#!/usr/bin/env bash
# Easy Reader Terminal - Dark Title Bar (Ghostty)
# Sets Ghostty to use the transparent macOS titlebar style so the titlebar
# blends with the dark terminal background.
...
DESIRED="$GHOSTTY_DARK_STYLE"

echo "=== Easy Reader Terminal: Dark Title Bar (Ghostty) ==="
echo ""
...
if [ "$CURRENT" = "$DESIRED" ]; then
    echo "Ghostty titlebar style is already set to transparent."
elif [ -n "$CURRENT" ]; then
    perl -0pi -e 's/^macos-titlebar-style = .*$/macos-titlebar-style = transparent/m' "$EASY_READER_FILE"
    echo "Set Ghostty titlebar style to transparent."
else
