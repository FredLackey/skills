...
const MANIFEST_SCHEMA = 'setup-multi-herdr/manifest-v3';
...
if [ -z "$herdr_bin" ] && command -v herdr >/dev/null 2>&1; then herdr_bin=$(command -v herdr); fi
if [ -n "$herdr_bin" ]; then
...
function usage() {
  return `Usage:
  setup-multi-herdr.mjs list-identities [--json]
  setup-multi-herdr.mjs manifest-digest --manifest FILE
  setup-multi-herdr.mjs bond --manifest FILE --approve sha256:HEX [--json] [--dry-run]
...
list-identities reports reusable local SSH keypairs by name and fingerprint.
manifest-digest is read-only. bond treats the approved manifest as authority to
trust first-seen keys for its exact SSH targets and reconcile their Herdr profiles.
Changed host keys remain blocked. The approved plan may bootstrap SSH access and
install or update Herdr, including replacing an incompatible remote server.
...
export function parseArgs(argv) {
  ...
  if (!['list-identities', 'manifest-digest', 'bond', 'preflight', 'discover', 'doctor', 'resolve', 'apply'].includes(command)) throw new Error(`Unknown command: ${command}`);
  const options = {
    ...
    report: null, remediation: null, approve: null, dryRun: false, host: false, manifest: null,
  };
  ...
  if (['preflight', 'discover', 'doctor'].includes(command) &&
      (options.proposal || options.report || options.remediation || options.approve || options.dryRun || options.manifest)) {
  ...
  if (command === 'resolve') {
    ...
    if (options.proposal || options.manifest || options.json || options.cidrs.length || options.interfaces.length || options.targets.length || options.host ||
  ...
  if (command === 'apply') {
    ...
    if (options.report || options.remediation || options.manifest || options.host || options.json || options.cidrs.length || options.interfaces.length || options.targets.length ||
        options.sshPort !== 22 || options.timeoutMs !== DEFAULT_TIMEOUT_MS || options.maxHosts !== DEFAULT_MAX_HOSTS) {
  ...
  if (command === 'manifest-digest') {
    if (!options.manifest) throw new Error('manifest-digest requires --manifest');
    if (options.proposal || options.report || options.remediation || options.approve || options.json || options.dryRun || options.host ||
        options.cidrs.length || options.interfaces.length || options.targets.length || options.sshPort !== 22 ||
        options.timeoutMs !== DEFAULT_TIMEOUT_MS || options.maxHosts !== DEFAULT_MAX_HOSTS) {
      throw new Error('unrelated options were supplied to manifest-digest');
    }
  }
  if (command === 'list-identities') {
    if (options.manifest || options.proposal || options.report || options.remediation || options.approve || options.dryRun || options.host ||
        options.cidrs.length || options.interfaces.length || options.targets.length || options.sshPort !== 22 ||
        options.timeoutMs !== DEFAULT_TIMEOUT_MS || options.maxHosts !== DEFAULT_MAX_HOSTS) {
      throw new Error('unrelated options were supplied to list-identities');
    }
  }
  if (command === 'bond') {
    if (!options.manifest || !options.approve) throw new Error('bond requires --manifest and --approve');
    if (options.proposal || options.report || options.remediation || options.host || options.cidrs.length || options.interfaces.length ||
        options.targets.length || options.sshPort !== 22 || options.timeoutMs !== DEFAULT_TIMEOUT_MS || options.maxHosts !== DEFAULT_MAX_HOSTS) {
      throw new Error('unrelated options were supplied to bond');
    }
    if (!/^sha256:[a-f0-9]{64}$/.test(options.approve)) throw new Error('invalid approval digest');
  }
  return options;
...
function localCEnvWithoutAgent() {
  const env = localCEnv();
...
function validateIdentityName(value) {
  if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(value) || value.endsWith('.pub')) {
    throw new Error(`invalid SSH identity name: ${JSON.stringify(value)}`);
  }
  return value;
}

function identityFiles(name, sshDirectory = path.join(os.homedir(), '.ssh')) {
  const identity = validateIdentityName(name);
  return { privateFile: path.join(sshDirectory, identity), publicFile: path.join(sshDirectory, `${identity}.pub`) };
}

export function listSshIdentities(sshDirectory = path.join(os.homedir(), '.ssh')) {
  let entries = [];
  try { entries = fs.readdirSync(sshDirectory, { withFileTypes: true }); } catch { return []; }
  ...
  for (const entry of entries) {
    ...
    let files;
    try { files = identityFiles(name, sshDirectory); } catch { continue; }
    let privateStat;
    try { privateStat = fs.statSync(files.privateFile); } catch { continue; }
    if (!privateStat.isFile()) continue;
    const fingerprint = spawnSync(sshKeygen, ['-lf', files.publicFile, '-E', 'sha256'], {
      shell: false, encoding: 'utf8', timeout: 3000, maxBuffer: 64 * 1024,
    });
    const match = fingerprint.status === 0
      ? fingerprint.stdout.trim().match(/^\d+\s+(SHA256:\S+).+\(([^)]+)\)$/)
      : null;
    if (!match) continue;
    identities.push({
      name,
      fingerprint: match[1],
      type: match[2],
      privateModeSecure: process.platform === 'win32' || (privateStat.mode & 0o077) === 0,
    });
  }
...
function resolveIdentity(name) {
  const files = identityFiles(name);
  const available = listSshIdentities().find((item) => item.name === name);
  if (!available) throw new Error(`SSH identity pair is unavailable: ${name}`);
  if (!available.privateModeSecure) throw new Error(`SSH identity private-file permissions are too broad: ${name}`);
  return { ...available, ...files };
}

export function validateBondManifest(value) {
  if (!value || value.schema !== MANIFEST_SCHEMA || !Array.isArray(value.machines)) {
    throw new Error(`bond manifest must use schema ${MANIFEST_SCHEMA}`);
  }
  const manifestKeys = new Set(['schema', 'hostKeyPolicy', 'remoteSetup', 'machines']);
  if (Object.keys(value).some((key) => !manifestKeys.has(key))) throw new Error('bond manifest contains an unknown field');
  if (value.machines.length < 1 || value.machines.length > 64) {
    throw new Error('bond manifest must contain between 1 and 64 machines');
  }
  if ((value.hostKeyPolicy ?? 'accept-new') !== 'accept-new') {
    throw new Error('bond manifest hostKeyPolicy must be accept-new');
  }
  if ((value.remoteSetup ?? 'install-or-update') !== 'install-or-update') {
    throw new Error('bond manifest remoteSetup must be install-or-update');
  }
  const machines = value.machines.map((machine) => {
    if (!machine || typeof machine !== 'object' || Array.isArray(machine)) throw new Error('invalid bond machine');
    const machineKeys = new Set(['target', 'label', 'session', 'identity', 'identityFingerprint', 'transport', 'tailscale', 'allowServerStop', 'bootstrap']);
    if (Object.keys(machine).some((key) => !machineKeys.has(key))) throw new Error('bond machine contains an unknown field');
    const target = validateTarget(machine.target);
    const label = clean(machine.label, 128);
    const session = clean(machine.session ?? 'default', 128);
    const identity = validateIdentityName(machine.identity);
    if (!/^SHA256:\S+$/.test(machine.identityFingerprint ?? '')) throw new Error(`identityFingerprint is required for ${target}`);
    const identityFingerprint = machine.identityFingerprint;
    const transport = machine.transport ?? 'ssh';
    if (!['ssh', 'tailscale'].includes(transport)) throw new Error(`invalid transport for ${target}`);
    const allowServerStop = machine.allowServerStop === true;
    let tailscale = null;
    if (transport === 'tailscale') {
      if (!machine.tailscale || typeof machine.tailscale !== 'object' || Array.isArray(machine.tailscale)) throw new Error(`tailscale policy is required for ${target}`);
      const tailscaleKeys = new Set(['expectedIp', 'ensureSsh']);
      if (Object.keys(machine.tailscale).some((key) => !tailscaleKeys.has(key))) throw new Error(`tailscale policy contains an unknown field for ${target}`);
      if (net.isIP(machine.tailscale.expectedIp) !== 4 || (machine.tailscale.ensureSsh != null && typeof machine.tailscale.ensureSsh !== 'boolean')) throw new Error(`invalid tailscale policy for ${target}`);
      tailscale = { expectedIp: machine.tailscale.expectedIp, ensureSsh: machine.tailscale.ensureSsh ?? false };
    } else if (machine.tailscale != null) throw new Error(`tailscale policy requires tailscale transport for ${target}`);
    const rawBootstrap = machine.bootstrap ?? { method: 'already-authorized' };
    if (!rawBootstrap || typeof rawBootstrap !== 'object' || Array.isArray(rawBootstrap)) throw new Error(`invalid bootstrap for ${target}`);
    const bootstrapKeys = new Set(['method', 'target', 'identityFile', 'identityFingerprint']);
    if (Object.keys(rawBootstrap).some((key) => !bootstrapKeys.has(key))) throw new Error(`bootstrap contains an unknown field for ${target}`);
    const method = rawBootstrap.method ?? 'already-authorized';
    if (!['already-authorized', 'management-identity', 'interactive'].includes(method)) throw new Error(`invalid bootstrap method for ${target}`);
    let bootstrap = { method };
    if (method !== 'already-authorized') {
      bootstrap.target = validateTarget(rawBootstrap.target);
    }
    if (method === 'management-identity') {
      if (typeof rawBootstrap.identityFile !== 'string' || !path.isAbsolute(rawBootstrap.identityFile)) {
        throw new Error(`bootstrap identityFile must be absolute for ${target}`);
      }
      if (!/^SHA256:\S+$/.test(rawBootstrap.identityFingerprint ?? '')) {
        throw new Error(`bootstrap identityFingerprint is required for ${target}`);
      }
      bootstrap.identityFile = path.normalize(rawBootstrap.identityFile);
      bootstrap.identityFingerprint = rawBootstrap.identityFingerprint;
    }
    if (!label || label !== machine.label || !session || (machine.session != null && session !== machine.session)) {
      throw new Error(`invalid label or session for ${target}`);
    }
    return { target, label, session, identity, identityFingerprint, transport, tailscale, allowServerStop, bootstrap };
  }).sort((a, b) => `${a.target}\u0000${a.session}`.localeCompare(`${b.target}\u0000${b.session}`));
  const targetSessions = new Set();
  const hostIdentities = new Map();
  const labels = new Set();
  for (const machine of machines) {
    const host = parseTarget(machine.target).host.toLowerCase();
    if (hostIdentities.has(host) && hostIdentities.get(host) !== machine.identity) throw new Error(`conflicting identities for SSH host: ${host}`);
    hostIdentities.set(host, machine.identity);
    const targetSession = `${machine.target}\u0000${machine.session}`;
    if (targetSessions.has(targetSession)) throw new Error(`duplicate bond target/session: ${machine.target} / ${machine.session}`);
    targetSessions.add(targetSession);
    const foldedLabel = machine.label.toLowerCase();
    if (labels.has(foldedLabel)) throw new Error(`duplicate bond label: ${machine.label}`);
...
export function bondManifestDigest(value) {
  return sha(canonicalJson(validateBondManifest(value)));
}
...
function run(command, args, { timeoutMs = 5000, maxOutput = MAX_OUTPUT, env = process.env, input = null } = {}) {
  return new Promise((resolve) => {
    let stdout = Buffer.alloc(0);
    let stderr = Buffer.alloc(0);
    let settled = false;
    let overflow = false;
    const child = spawn(command, args, { shell: false, env, stdio: [input == null ? 'ignore' : 'pipe', 'pipe', 'pipe'] });
    const finish = (result) => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
...
function sshArgs(target, defaultPort, remoteProgram, options = {}) {
  ...
  return [
    '-T', '-S', 'none', '-o', 'BatchMode=yes', '-o', 'NumberOfPasswordPrompts=0',
    '-o', 'KbdInteractiveAuthentication=no', '-o', `StrictHostKeyChecking=${hostKeyPolicy === 'accept-new' ? 'accept-new' : 'yes'}`,
    ...
    ...(options.identityFile ? ['-o', 'IdentitiesOnly=yes', '-i', options.identityFile] : []),
...
async function sshProbe(route, sshExecutable, nonce, options) {
  ...
  const result = await run(sshExecutable, sshArgs(
    route.target, route.port ?? options.sshPort, REMOTE_PROBE,
    { hostKeyPolicy: options.hostKeyPolicy, identityFile: options.identityFile },
  ), {
    timeoutMs: 6000, maxOutput: 64 * 1024, env: options.env ?? localCEnv(),
  });
...
function loadPrivateJson(filePath, label = 'file') {
...
function loadPrivateReport(filePath, label = 'report') {
  const report = loadPrivateJson(filePath, label);
  if (report?.schema !== SCHEMA) throw new Error(`unsupported ${label} schema`);
  return report;
}

function loadBondManifest(filePath) {
  return validateBondManifest(loadPrivateJson(filePath, 'bond manifest'));
}
...
function runHerdrNonInteractive(executable, args, env = localCEnv()) {
...
function keyFingerprint(file) {
  const sshKeygen = findExecutable('ssh-keygen');
  if (!sshKeygen) throw new Error('ssh-keygen is unavailable');
  const result = spawnSync(sshKeygen, ['-lf', file, '-E', 'sha256'], {
    shell: false, encoding: 'utf8', timeout: 5000, maxBuffer: 64 * 1024,
  });
  const match = result.status === 0 ? result.stdout.match(/\b(SHA256:\S+)/) : null;
  if (!match) throw new Error(`cannot fingerprint SSH identity: ${file}`);
  return match[1];
}

function stageBootstrapIdentity(bootstrap) {
  const source = bootstrap.identityFile;
  const stat = fs.lstatSync(source);
  if (!stat.isFile() || stat.isSymbolicLink()) throw new Error('bootstrap identity must be a regular, non-symlink file');
  if (process.getuid && stat.uid !== process.getuid()) throw new Error('bootstrap identity must be owned by the current user');
  const actual = keyFingerprint(source);
  if (actual !== bootstrap.identityFingerprint) throw new Error('bootstrap identity fingerprint does not match the approved manifest');
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-multi-herdr-key-'));
  fs.chmodSync(directory, 0o700);
  const staged = path.join(directory, 'identity');
  fs.copyFileSync(source, staged);
  fs.chmodSync(staged, 0o600);
  if (keyFingerprint(staged) !== bootstrap.identityFingerprint) {
    fs.rmSync(directory, { recursive: true, force: true });
    throw new Error('staged bootstrap identity fingerprint changed during copy');
  }
  return { file: staged, cleanup: () => fs.rmSync(directory, { recursive: true, force: true }) };
}

async function installPublicKey(machine, identity, sshExecutable) {
  if (machine.bootstrap.method === 'already-authorized') return { ok: false, skipped: true };
  if (machine.bootstrap.method === 'interactive') {
    const copyId = findExecutable('ssh-copy-id');
    if (!copyId) return { ok: false, reason: 'ssh-copy-id is unavailable' };
    if (!process.stdin.isTTY || !process.stderr.isTTY) return { ok: false, reason: 'interactive SSH bootstrap requires a terminal' };
    const result = spawnSync(copyId, [
      '-o', 'StrictHostKeyChecking=accept-new', '-i', identity.publicFile, machine.bootstrap.target,
    ], { shell: false, stdio: 'inherit' });
    return result.status === 0 ? { ok: true } : { ok: false, reason: 'ssh-copy-id failed' };
  }
  let staged;
  try {
    staged = stageBootstrapIdentity(machine.bootstrap);
    const publicKey = fs.readFileSync(identity.publicFile, 'utf8').trim();
    if (!publicKey || /[\r\n]/.test(publicKey)) throw new Error('selected public key is malformed');
    const command = "umask 077; mkdir -p \"$HOME/.ssh\"; touch \"$HOME/.ssh/authorized_keys\"; chmod 700 \"$HOME/.ssh\"; chmod 600 \"$HOME/.ssh/authorized_keys\"; IFS= read -r key; grep -qxF \"$key\" \"$HOME/.ssh/authorized_keys\" || printf '%s\\n' \"$key\" >> \"$HOME/.ssh/authorized_keys\"";
    const result = await run(sshExecutable, sshArgs(machine.bootstrap.target, 22, command, {
      hostKeyPolicy: 'accept-new', identityFile: staged.file,
    }), { timeoutMs: 30000, maxOutput: 64 * 1024, env: localCEnv(), input: `${publicKey}\n` });
    return result.code === 0 && !result.overflow && !result.timedOut
      ? { ok: true }
      : { ok: false, reason: clean(result.stderr || 'management SSH bootstrap failed', 600) };
  } catch (error) {
...
async function ensureTailscaleTransport(machine, identity, sshExecutable) {
  if (machine.transport !== 'tailscale') return { ok: true, changed: false };
  if (!machine.tailscale.ensureSsh) return { ok: true, changed: false };
  const command = "expected=$1; actual=$(tailscale ip -4 2>/dev/null | sed -n '1p'); [ \"$actual\" = \"$expected\" ] || { printf '%s\\n' \"expected Tailscale IP $expected but found ${actual:-none}\" >&2; exit 21; }; tailscale set --ssh=false";
  const remoteProgram = `sh -c ${shellQuote(command)} sh ${shellQuote(machine.tailscale.expectedIp)}`;
  let staged;
  try {
    const useManagement = machine.bootstrap.method === 'management-identity';
    if (useManagement) staged = stageBootstrapIdentity(machine.bootstrap);
    const target = useManagement ? machine.bootstrap.target : machine.target;
    const identityFile = useManagement ? staged.file : identity.privateFile;
    const result = await run(sshExecutable, sshArgs(target, 22, remoteProgram, {
      hostKeyPolicy: 'accept-new', identityFile,
    }), { timeoutMs: 30000, maxOutput: 64 * 1024, env: localCEnv() });
    return result.code === 0 && !result.overflow && !result.timedOut
      ? { ok: true, changed: true }
      : { ok: false, reason: clean(`${result.stderr}\n${result.stdout}`.trim(), 800) || 'Tailscale SSH reconciliation failed' };
  } catch (error) {
    return { ok: false, reason: error.message };
  } finally {
    staged?.cleanup();
...
function versionNumber(value) {
  return String(value ?? '').match(/\b(\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?)/)?.[1] ?? null;
}

function incompatibleVersion(localVersion, remoteVersion) {
  const local = versionNumber(localVersion)?.split(/[.+-]/).slice(0, 2).join('.');
  const remote = versionNumber(remoteVersion)?.split(/[.+-]/).slice(0, 2).join('.');
  return Boolean(local && remote && local !== remote);
}

async function targetResolvesTo(targetHost, expectedIp) {
  if (!targetHost || !expectedIp) return false;
  if (targetHost === expectedIp) return true;
  try {
    const addresses = await dns.lookup(targetHost, { all: true, family: 4 });
...
const REMOTE_INSTALL_HERDR = String.raw`set -eu
session=$1
stop_server=$2
find_herdr() {
  for candidate in "$HOME/.local/bin/herdr" "$HOME/.local/share/mise/shims/herdr" "$HOME/.nix-profile/bin/herdr" /opt/homebrew/bin/herdr /usr/local/bin/herdr /usr/bin/herdr; do
    if [ -x "$candidate" ]; then printf '%s\n' "$candidate"; return 0; fi
  done
  if command -v herdr >/dev/null 2>&1; then command -v herdr; return 0; fi
  return 1
}
old_bin=$(find_herdr || true)
if [ "$stop_server" = true ] && [ -n "$old_bin" ]; then
  "$old_bin" --session "$session" server stop >/dev/null 2>&1 || "$old_bin" server stop >/dev/null 2>&1 || {
    printf '%s\n' 'authorized incompatible Herdr server stop failed' >&2
    exit 22
  }
fi
installer=$(mktemp /tmp/herdr-install.XXXXXX)
trap 'rm -f "$installer"' EXIT HUP INT TERM
if command -v curl >/dev/null 2>&1; then
  curl -fsSL https://herdr.dev/install.sh -o "$installer"
elif command -v wget >/dev/null 2>&1; then
  wget -qO "$installer" https://herdr.dev/install.sh
else
  printf '%s\n' 'curl or wget is required to install Herdr' >&2
  exit 20
fi
sh "$installer"
new_bin=$(find_herdr)
...
async function ensureRemoteHerdr(machine, identity, probe, localVersion, sshExecutable) {
  const local = versionNumber(localVersion);
  const remote = versionNumber(probe.herdrVersion);
  if (local && remote === local) return { ok: true, changed: false };
  const needsStop = probe.herdrRunning && incompatibleVersion(localVersion, probe.herdrVersion);
  if (needsStop && !machine.allowServerStop) return { ok: false, reason: 'an incompatible running Herdr server requires allowServerStop in the approved manifest' };
  const stop = needsStop && machine.allowServerStop;
  const remoteCommand = `sh -s -- ${shellQuote(machine.session)} ${stop ? 'true' : 'false'}`;
  const result = await run(sshExecutable, sshArgs(machine.target, parseTarget(machine.target, 22).port, remoteCommand, {
    hostKeyPolicy: 'accept-new', identityFile: identity.privateFile,
  }), { timeoutMs: 180000, maxOutput: MAX_OUTPUT, env: localCEnv(), input: REMOTE_INSTALL_HERDR });
  return result.code === 0 && !result.overflow && !result.timedOut
    ? { ok: true, changed: true, stoppedIncompatibleServer: stop, version: clean(result.stdout, 160) }
    : { ok: false, reason: clean(`${result.stderr}\n${result.stdout}`.trim(), 1200) || 'remote Herdr installation failed' };
}

function withEphemeralAgent(identityFile, callback) {
  const sshAgent = findExecutable('ssh-agent');
  const sshAdd = findExecutable('ssh-add');
  const realSsh = findExecutable('ssh');
  if (!sshAgent || !sshAdd || !realSsh) return { ok: false, status: null, output: 'ssh, ssh-agent, and ssh-add are required for unattended Herdr catalog setup' };
  const wrapperDirectory = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-multi-herdr-ssh-'));
  fs.chmodSync(wrapperDirectory, 0o700);
  const wrapper = path.join(wrapperDirectory, 'ssh');
  fs.writeFileSync(wrapper, `#!/bin/sh\nexec ${shellQuote(realSsh)} -o IdentitiesOnly=yes -i ${shellQuote(identityFile)} "$@"\n`, { mode: 0o700 });
  const started = spawnSync(sshAgent, ['-s'], { shell: false, encoding: 'utf8', timeout: 5000 });
  const socket = /SSH_AUTH_SOCK=([^;\n]+)/.exec(started.stdout)?.[1];
  const pid = /SSH_AGENT_PID=(\d+)/.exec(started.stdout)?.[1];
  if (started.status !== 0 || !socket || !pid) {
    fs.rmSync(wrapperDirectory, { recursive: true, force: true });
    return { ok: false, status: started.status, output: 'could not start an isolated SSH agent' };
  }
  const env = localCEnv({ SSH_AUTH_SOCK: socket, SSH_AGENT_PID: pid, PATH: `${wrapperDirectory}${path.delimiter}${process.env.PATH ?? ''}` });
  try {
    const added = spawnSync(sshAdd, [identityFile], { shell: false, encoding: 'utf8', env, timeout: 15000 });
    if (added.status !== 0) return { ok: false, status: added.status, output: clean(added.stderr, 1200) };
    return callback(env);
  } finally {
    spawnSync(sshAgent, ['-k'], { shell: false, encoding: 'utf8', env, timeout: 5000 });
    fs.rmSync(wrapperDirectory, { recursive: true, force: true });
  }
}

async function bondMachines(options) {
  const progress = (message) => process.stderr.write(`[setup-multi-herdr] ${message}\n`);
  const manifest = loadBondManifest(options.manifest);
  const digest = bondManifestDigest(manifest);
  if (digest !== options.approve) {
    throw Object.assign(new Error('approval digest does not match the bond manifest'), { exitCode: 4 });
  }
  const preflight = await preflightReport();
  if (preflight.outcome !== 'ready') return {
    schema: MANIFEST_SCHEMA,
  ...
  const sshExecutable = findExecutable('ssh');
  const initial = await inspectHerdr({ includeInstallMethod: false });
  const nonce = crypto.randomBytes(16).toString('hex');
  const results = [];

  for (const machine of manifest.machines) {
    progress(`${machine.label}: starting (${machine.target})`);
    let identity;
    try { identity = resolveIdentity(machine.identity); }
    catch (error) {
      results.push({
        ...machine,
        status: 'blocked',
        stage: 'client-identity',
        reasonCode: 'ssh.identity.unavailable',
        summary: error.message,
        actor: 'local-user',
      });
      continue;
    }
    progress(`${machine.label}: verified ${machine.identity} (${identity.fingerprint})`);
    if (identity.fingerprint !== machine.identityFingerprint) {
      results.push({
        ...machine, status: 'blocked', stage: 'client-identity', reasonCode: 'ssh.identity.fingerprint-drift',
        summary: `The selected ${machine.identity} key no longer matches the approved fingerprint.`, actor: 'local-user',
      });
      continue;
    }
    const port = parseTarget(machine.target, 22).port;
    if (!options.dryRun) {
      try {
        reconcileSshIdentity(path.join(os.homedir(), '.ssh', 'config'), parseTarget(machine.target, port).host, identity.privateFile);
      } catch (error) {
        results.push({ ...machine, status: 'blocked', stage: 'client-persistence',
          reasonCode: 'ssh.config.write-failed', summary: error.message, actor: 'local-user' });
        continue;
      }
    }
    let bootstrapAttempt = null;
    if (!options.dryRun && machine.bootstrap.method === 'management-identity') {
      progress(`${machine.label}: authorizing the Herdr key through the approved management route`);
      bootstrapAttempt = await installPublicKey(machine, identity, sshExecutable);
      if (!bootstrapAttempt.ok) {
        results.push({
          ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', stage: 'ssh-bootstrap',
          reasonCode: 'ssh.authorization.bootstrap-failed', actor: 'local-user-or-target-admin',
          summary: bootstrapAttempt.reason,
        });
        continue;
      }
    }
    if (!options.dryRun && machine.transport === 'tailscale') {
      progress(`${machine.label}: reconciling the approved Tailscale SSH route`);
      const tailscaleSetup = await ensureTailscaleTransport(machine, identity, sshExecutable);
      if (!tailscaleSetup.ok) {
        results.push({
          ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', stage: 'tailscale',
          reasonCode: 'tailscale.ssh-reconciliation-failed', actor: 'target-owner-or-tailnet-admin',
          summary: tailscaleSetup.reason,
        });
        continue;
      }
    }
    progress(`${machine.label}: verifying passwordless SSH and remote state`);
    let probe = await sshProbe(
      { target: machine.target, port },
      sshExecutable,
      nonce,
      {
        sshPort: port,
        timeoutMs: DEFAULT_TIMEOUT_MS,
        hostKeyPolicy: options.dryRun ? 'strict' : manifest.hostKeyPolicy,
        identityFile: identity.privateFile,
      },
    );
    if (!probe.authenticated && ['ssh.authentication.rejected', 'ssh.authentication.interactive-required'].includes(probe.reasonCode)) {
      const bootstrap = bootstrapAttempt ?? await installPublicKey(machine, identity, sshExecutable);
      if (bootstrap.ok) {
        probe = await sshProbe(
          { target: machine.target, port }, sshExecutable, nonce,
          { sshPort: port, timeoutMs: DEFAULT_TIMEOUT_MS, hostKeyPolicy: manifest.hostKeyPolicy, identityFile: identity.privateFile },
        );
    ...
    if (!probe.authenticated) {
      const identityRejected = ['ssh.authentication.rejected', 'ssh.authentication.interactive-required'].includes(probe.reasonCode);
      results.push({
        ...machine,
        identityFingerprint: identity.fingerprint,
        status: 'blocked',
        stage: probe.stage,
        reasonCode: identityRejected ? 'ssh.identity.not-authorized' : probe.reasonCode,
        summary: identityRejected
          ? `The selected ${machine.identity} key (${identity.fingerprint}) was offered explicitly but the target did not authorize it.`
          : probe.summary,
        actor: identityRejected ? 'local-user-or-target-admin' : issueActor(probe.reasonCode),
      });
      continue;
    }
    const usesApprovedTailscaleRoute = machine.transport !== 'tailscale' ||
      (probe.tailscaleIp === machine.tailscale.expectedIp &&
        await targetResolvesTo(probe.effective?.hostname ?? parseTarget(machine.target, port).host, machine.tailscale.expectedIp));
    if (machine.transport === 'tailscale' && !usesApprovedTailscaleRoute) {
      results.push({
        ...machine,
        identityFingerprint: identity.fingerprint,
        status: 'blocked',
        stage: 'tailscale',
        reasonCode: 'tailscale.remote-not-connected',
        actor: 'target-owner-or-tailnet-admin',
        summary: 'The approved target requires Tailscale, but its effective SSH hostname did not resolve to the authenticated remote Tailscale IPv4 address.',
      });
      continue;
    }
    if (probe.state === 'unsupported-platform') {
      results.push({ ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', stage: probe.stage,
        reasonCode: probe.reasonCode, actor: issueActor(probe.reasonCode), summary: probe.summary });
      continue;
    }
    if (options.dryRun) {
      progress(`${machine.label}: dry-run complete; no remote or catalog changes made`);
      results.push({
        ...machine, identityFingerprint: identity.fingerprint, status: 'dry-run', actions: [],
        remoteSetup: { planned: versionNumber(probe.herdrVersion) !== versionNumber(initial.version), allowServerStop: machine.allowServerStop },
      });
      continue;
    }
    progress(`${machine.label}: ensuring stable Herdr is installed and compatible`);
    const remoteSetup = await ensureRemoteHerdr(machine, identity, probe, initial.version, sshExecutable);
    if (!remoteSetup.ok) {
      results.push({
        ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', stage: 'remote-setup',
        reasonCode: 'remote.herdr.install-or-update-failed', actor: 'target-owner-or-network', summary: remoteSetup.reason,
      });
      continue;
    }
    if (remoteSetup.changed) {
      probe = await sshProbe(
        { target: machine.target, port }, sshExecutable, nonce,
        { sshPort: port, timeoutMs: DEFAULT_TIMEOUT_MS, hostKeyPolicy: manifest.hostKeyPolicy, identityFile: identity.privateFile },
      );
      if (!probe.authenticated || !['herdr-running', 'herdr-installed'].includes(probe.state) || !versionNumber(probe.herdrVersion)) {
        results.push({
          ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', stage: 'remote-setup',
          reasonCode: 'remote.herdr.post-install-verification-failed', actor: 'target-owner-or-herdr',
          summary: 'The remote Herdr install/update command completed, but a fresh fixed probe did not find a usable Herdr binary.',
        });
        continue;
      }
    }

    const snapshot = await inspectHerdr({ includeInstallMethod: false });
    const target = canonicalHerdrTarget(machine.target, port);
    let profile = snapshot.profiles.find((item) => item.target === target && item.session === machine.session) ?? null;
    const collision = snapshot.profiles.find((item) => item.label.toLowerCase() === machine.label.toLowerCase() && item.id !== profile?.id);
    if (collision) {
      results.push({
        ...machine,
        status: 'blocked',
        stage: 'catalog',
        reasonCode: 'catalog.label-collision',
        actor: 'local-user',
        summary: `Another saved profile already uses label ${machine.label}.`,
      });
      continue;
    }

    const actions = [];
    if (!profile) actions.push({ type: 'add', target, label: machine.label, session: machine.session });
    else {
      if (profile.label !== machine.label) actions.push({ type: 'rename', profileId: profile.id, label: machine.label });
      if (!profile.enabled) actions.push({ type: 'enable', profileId: profile.id });
    }

    let failure = null;
    for (const action of actions) {
      progress(`${machine.label}: applying catalog action ${action.type}`);
      const args = action.type === 'add'
        ? ['machine', 'add', action.target, '--label', action.label,
          ...(action.session !== 'default' ? ['--remote-session', action.session] : [])]
        : action.type === 'rename'
          ? ['machine', 'rename', action.profileId, '--label', action.label]
          : ['machine', 'enable', action.profileId];
      const applied = withEphemeralAgent(identity.privateFile, (env) => runHerdrNonInteractive(initial.executable, args, env));
      if (!applied.ok) {
        failure = {
          stage: 'remote-setup',
          reasonCode: action.type === 'add' ? 'remote.setup.failed-after-bootstrap' : 'catalog.command-failed',
          actor: 'local-user-or-herdr',
          summary: action.type === 'add'
            ? 'Herdr could not add this machine after the approved SSH and remote Herdr setup completed.'
            : `Herdr ${action.type} failed.`,
          diagnostic: applied.output || null,
        };
        break;
      }
      const after = await inspectHerdr({ includeInstallMethod: false });
      profile = after.profiles.find((item) => item.target === target && item.session === machine.session) ?? profile;
    }
    if (failure) {
      results.push({ ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', actions, ...failure });
      continue;
    }
    const verified = await inspectHerdr({ includeInstallMethod: false });
    const finalProfile = verified.profiles.find((item) => item.target === target && item.session === machine.session);
    if (!finalProfile || finalProfile.label !== machine.label || !finalProfile.enabled) {
      results.push({
        ...machine,
        identityFingerprint: identity.fingerprint,
        status: 'blocked',
        actions,
        stage: 'verification',
        reasonCode: 'catalog.verification-failed',
        actor: 'local-user-or-herdr',
        summary: 'The saved profile did not match the approved manifest after reconciliation.',
      });
      continue;
    }
    progress(`${machine.label}: verifying the persistent client SSH path used by Herdr`);
    const persistentProbe = await sshProbe(
      { target: machine.target, port }, sshExecutable, nonce,
      {
        sshPort: port, timeoutMs: DEFAULT_TIMEOUT_MS, hostKeyPolicy: 'strict',
        env: localCEnvWithoutAgent(),
      },
    );
    if (!persistentProbe.authenticated || !['herdr-running', 'herdr-installed'].includes(persistentProbe.state)) {
      results.push({
        ...machine, identityFingerprint: identity.fingerprint, status: 'blocked', actions,
        stage: 'client-persistence', reasonCode: 'ssh.persistence.not-ready', actor: 'local-user',
        summary: 'The catalog entry exists, but ordinary agentless SSH cannot authenticate with the persistent client configuration Herdr will use.',
        diagnostic: persistentProbe.summary,
      });
      continue;
    }
    const status = actions.length ? 'bonded' : 'already-bonded';
    progress(`${machine.label}: ${status}`);
    results.push({ ...machine, identityFingerprint: identity.fingerprint, status, actions, remoteSetup });
  }

  const finalSnapshot = await inspectHerdr({ includeInstallMethod: false });
  const failed = results.filter((item) => item.status === 'blocked').length;
  return {
    schema: MANIFEST_SCHEMA,
    status: options.dryRun ? 'dry-run' : failed ? 'partial' : 'bonded',
    manifestDigest: digest,
    hostKeyPolicy: manifest.hostKeyPolicy,
    remoteSetup: manifest.remoteSetup,
    machines: results,
    finalProfiles: finalSnapshot.profiles,
  };
}

function printBondResult(report) {
  console.log(`Setup Multi-Herdr bond: ${report.status}`);
  for (const machine of report.machines) {
    console.log(`\n${machine.label} (${machine.target}): ${machine.status}`);
    if (machine.reasonCode) console.log(`  ${machine.stage} / ${machine.reasonCode}: ${machine.summary}`);
    if (machine.actor) console.log(`  Next owner: ${machine.actor}`);
    if (machine.diagnostic) console.log(`  ${machine.diagnostic}`);
    for (const action of machine.actions ?? []) console.log(`  ${action.type}: ${action.label ?? action.profileId ?? action.target}`);
  }
  console.log(`\nSaved profiles: ${report.finalProfiles.length}`);
}
...
async function main(argv = process.argv.slice(2)) {
  ...
  try {
    if (options.command === 'list-identities') {
      const identities = listSshIdentities();
      if (options.json) process.stdout.write(`${JSON.stringify(identities, null, 2)}\n`);
      else if (!identities.length) console.log('No reusable SSH identity pairs were found.');
      else for (const identity of identities) {
        console.log(`${identity.name}\t${identity.type}\t${identity.fingerprint}\t${identity.privateModeSecure ? 'secure' : 'insecure-permissions'}`);
      }
      return 0;
    }
    if (options.command === 'manifest-digest') {
      console.log(bondManifestDigest(loadBondManifest(options.manifest)));
      return 0;
    }
    if (options.command === 'bond') {
      const report = await bondMachines(options);
      if (options.json) process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
      else printBondResult(report);
      return ['bonded', 'dry-run'].includes(report.status) ? 0 : 5;
    }
